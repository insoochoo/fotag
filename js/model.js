...
function createModelModule() {
    ...
    _.extend(ImageModel.prototype, {
        ...
        setCaption: function(caption) {
            this.caption = caption;
            for(var i = 0; i < this.listeners.length; i++){
                this.listeners[i](this, new Date());
            }
        },
        ...
        setRating: function(rating) {
            ...
            var self = this;
            for(var i = 0; i < this.listeners.length; i++){
                this.listeners[i](self, new Date());
            }
        },
    ...
    _.extend(ImageCollectionModel.prototype, {
        ...
        addImageModel: function(imageModel) {
            ...
            var self = this;
            imageModel.addListener(function(image, eventTime){
                _.each(self.listeners, function(listener){
                    listener(IMAGE_META_DATA_CHANGED_EVENT, self, image, new Date());
                });
                storeImageCollectionModel(self);
            });

            _.each(this.listeners, function(listener){
                listener(IMAGE_ADDED_TO_COLLECTION_EVENT, self, imageModel, new Date());
            });
        },
        ...
        removeImageModel: function(imageModel) {
            var self = this;
            imageModel.removeListener(function(image, eventTime){
                _.each(self.listeners, function(listener){
                    listener(IMAGE_META_DATA_CHANGED_EVENT, self, image, new Date());
                })
            });

            _.each(this.imageModels, function(iM, index){
                if(iM === imageModel){
                    self.imageModels.splice(index,1);
                }
            });

            _.each(this.listeners, function(listener){
                listener(IMAGE_REMOVED_FROM_COLLECTION_EVENT, self, imageModel, new Date());
            });
